...
2. `PROGRESS.md`
3. `TASKS.md`
4. The assigned task card and its linked protocol documents

Read `DECISIONS.md`, `ARCHITECTURE.md`, and `MEMORY.md` only when they affect
the current scope. Do not treat this compact context kit as a replacement for
repository evidence.

## Operating Rules

- The repo-first task board is authoritative for task status.
- Work only on assigned, scoped tasks; record blockers as incidents.
